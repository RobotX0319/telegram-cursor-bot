...
🎛 Admin panel — chap pastdagi «🎛 Admin panel» yoki «🌐 Web panel» tugmasi
...
export async function handleAdminBotMessage(
  ...
  await grantAdminAccess(env, userId);
  ...
  if (manualId !== null) {
      ...
      await sendMessage(
        ...
        { bot: adminMsgBot.get(userId) ?? replyBot(env, userId), replyMarkup: adminPanelKeyboard(env, workerOrigin) },
      );
    return;
  }
  ...
  await sendMessage(
    ...
    chatId,
    [
      "🎛 Admin panel: /panel yoki pastdagi tugmalar",
      "",
      "📤 Video: ID yuboring (5), keyin video fayl",
      "/help — yordam",
    ].join("\n"),
    {
    ...
    case "/start":
      await sendMessage(
        ...
        [
          ...
          "",
          "📤 Video yuklash: avval ID (masalan 5), keyin video fayl",
